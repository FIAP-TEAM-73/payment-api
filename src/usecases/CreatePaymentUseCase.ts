...
import { HttpResponse, internalServerError, ok } from "../presenters/HttpResponses";
...
export class CreatePaymentUseCase {
    ...
    async execute(command: CreatePaymentCommand): Promise<HttpResponse> {
        try {
            ...
            const message = 'Error while creating a payment integration'
            console.log('Error while creating a payment integration', error instanceof Error ? error.message : JSON.stringify(error));
